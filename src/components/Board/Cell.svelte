...
    }
  ...
  function handleDoubleClick(event) {
	...
	userGrid.subscribe($userGrid => {
		if($userGrid[$cursor.y][$cursor.x]===0&&candidates1&&candidates1.length===1){
...
<div class="cell row-start-{cellY} col-start-{cellX}"
  ...
  {#if !disabled}
    <div class="cell-inner"
      ...
      <button class="cell-btn" on:click={handleClick} class:showhighlight={candidates1 && candidates1.length === 1}>
        {#if candidates1 && candidates1.length > 1 && hintsLevel>=$num&&$num>= candidates1.length}
          <Candidates {candidates1} cellY={cellY} cellX={cellX} />
        {:else}
          <span class="cell-text" class:showhigh={candidates1 && candidates1.length === 1} >{(candidates1 && candidates1.length === 1)?candidates1[0]:(value || '')}</span>
        {/if}
      </button>
    </div>
  {/if}
