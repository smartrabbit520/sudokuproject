...
function printSudoku(sudoku) {
    // 定义单元格的宽度和分隔符
    const cellWidth = 9; // 调整此值以适应不同长度的候选数列表
    const separator = ' | ';
            ...
            for (let j = 0; j < 9; j++) {
                ...
                for (let k = 0; k < 9; k++) {
                    ...
                    if (b3List.length === 1) {
                        ...
                        for (let row = 0; row < 9; row++) {
                            for (let col = 0; col < 9; col++) {
                                if (getBoxIndex(row, col) === getBoxIndex(i, k) && row === r) {
    ...
    solver(sudoku) {
        ...
        for (let val = 1; val <= 9; val++) {
            ...
            for (let j = 0; j < 9; j++) {
                let candidates = [];
                for (let i = 0; i < 9; i++) {
                    if (sudoku[i][j].size > 1 && sudoku[i][j].has(val)) {
                        candidates.push([i, j]);
                    }
                }
                if (candidates.length !== 2) continue;
                ...
                // 判断其他列是否有相同的候选值
                for (let otherJ = j + 1; otherJ < 9; otherJ++) {
                    if (sudoku[row1][otherJ].size > 1 && sudoku[row1][otherJ].has(val) && sudoku[row2][otherJ].size > 1 && sudoku[row2][otherJ].has(val)) {
                        // 如果其他列有相同的候选值，就删除这两列的其他单元格的候选值
