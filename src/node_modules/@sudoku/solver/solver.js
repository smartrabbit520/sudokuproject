...
class SimpleSolver extends BaseSolver {
    ...
    getUsedNumbers(sudoku, row, col) {
        ...
        return new Set([...sudoku[row][col]].filter(x => !allNumbers.has(x)));
    }
}
    ...
    solver(sudoku) {
        ...
        for (let val = 1; val <= 9; val++) {
            ...
            for (let j = 0; j < 9; j++) {
                ...
                // 判断其他列是否有相同的候选值
                for (let otherJ = 8; otherJ >= j + 1; otherJ--) {
    ...
    solver(sudoku) {
        ...
        for (let boxIndex = 0; boxIndex < 9; boxIndex++) {
            ...
            for (let val = 1; val <= 9; val++) {
                ...
                // 如果都在同一行，删除隔壁两个宫中该行的候选值
                const row = positions[0][0];

                for (let j = 0; j < 9; j++) {
                    // 如果在同一个宫中，就不删除
                    if (getBoxIndex(row, j) === boxIndex) continue;
                    if (sudoku[row][j].size === 1) continue;
