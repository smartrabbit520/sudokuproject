...
let sudokuArrays = [sudokuArray0, sudokuArray1, sudokuArray2];
...
for (let i = 0; i < 10; i++) {
    ...
    sudoku = runAllSolversOnce(sudoku);
...
示例代码中包含三个测试数独，下标记作0,1,2,3,4,5

对于0,3,4,5 方法都可以迭代出最终结果

对于1,2 方法无法获得最终结果，需要配合回溯来实现

使用时候，修改`arrayIndex`即可切换测试的数独