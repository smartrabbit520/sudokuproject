...
export function solveSudoku(sudoku) {
	...
	for (let cell = 0; cell < GRID_LENGTH; cell++) {
		...
		let pos = { x: col, y: row };
		if (sudoku1[row][col].size === 1 && before[row][col].size > 1) {//比较决策前后有些位置是否产生了唯一候选值
			grid[row][col] = 1;
		}
	...
	// 判断sudoku1候选值是否全为1-9，即所有策略失效
	let flag1 = true;
	let flag2 = true;	
	for (let i = 0; i < 9; i++) {
		for (let j = 0; j < 9; j++) {
			if (before[i][j].size !== 1) {
				if (sudoku1[i][j].size !== 9) {
					flag1 = false;
				}
			}
			if (sudoku1[i][j].size !== 1) {
				flag2 = false;
			}
		}
	}
